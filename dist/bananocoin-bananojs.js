...
(function() {
   ...
   * @memberof BananoUtil
   * @param {string} publicKey the public key.
   * @param {string} accountPrefix the prefix. ban_ or btco_.
  ...
  */
  ...
  const getNanoAccountValidationInfo = (account) => {
    ...
    if (!isValid) {
      return {
        message: `Invalid NANO account (characters after btco_ must be one of:13456789abcdefghijkmnopqrstuwxyz)`,
        valid: false,
      };
    }
...
(function() {
  ...
  const BITCOINNANO_PREFIX = 'btco_';
  ...
  const receiveNanoDepositsForSeed = async (
    ...
    const privateKey = bananoUtil.getPrivateKey(seed, seedIx);
    const publicKey = await bananoUtil.getPublicKey(privateKey);
    const account = bananoUtil.getAccount(publicKey, BITCOINNANO_PREFIX);
  ...
  const getRawStrFromNanoStr = (amountStr) => {
    return bananoUtil.getRawStrFromMajorAmountStr(amountStr, BITCOINNANO_PREFIX);
  };
  ...
  const getRawStrFromNanoshiStr = (amountStr) => {
    return bananoUtil.getRawStrFromMinorAmountStr(amountStr, BITCOINNANO_PREFIX);
  };
  ...
  const getNanoAccount = (publicKey) => {
    return bananoUtil.getAccount(publicKey, BITCOINNANO_PREFIX);
  };
  ...
  const getNanoPartsFromRaw = (amountRawStr) => {
    return bananoUtil.getAmountPartsFromRaw(amountRawStr, BITCOINNANO_PREFIX);
  };
  ...
  const exports = (() => {
    ...
    exports.BITCOINNANO_PREFIX = BITCOINNANO_PREFIX;
    exports.PREFIXES = [BANANO_PREFIX, BITCOINNANO_PREFIX];
